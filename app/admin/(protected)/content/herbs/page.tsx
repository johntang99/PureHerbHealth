...
export default function HerbProfilesPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    const params = new URLSearchParams({ type: "herb_profile", per_page: "100" });
  ...
  async function createHerb() {
    ...
    router.push(`/admin/content/herbs/${data.item.id}`);
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className={`text-xl font-bold ${t.heading}`}>Herb Profiles</h1>
          <p className={`text-[13px] ${t.muted}`}>{total} herb profiles · TCM education content</p>
        </div>
      ...
      {newForm && (
        <div className="rounded-xl border border-[#2D8C54]/30 bg-green-50 p-5 shadow-sm">
          <p className="mb-3 text-[13px] font-bold text-[#2D8C54]">New Herb Profile</p>
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
              <label className={`mb-1 block ${t.labelClass}`}>Herb name (EN) *</label>
              <input
                type="text"
                placeholder="e.g. Astragalus Root"
                autoFocus
                value={newForm.title}
          ...
          <div className="mt-3 flex gap-2">
            <button type="button" disabled={newForm.saving} onClick={() => void createHerb()} className={t.btnPrimary}>
      ...
      <div className="flex flex-wrap gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
        <input
          type="text"
          placeholder="Search herb profiles…"
          value={search}
      ...
      {loading && <div className={t.alertLoading}>Loading herb profiles…</div>}
          ...
          <table className="min-w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                {["Herb", "Slug", "Status", "Views", "Published", "Updated", ""].map((h) => (
                ...
                <tr key={item.id} className={t.tableRow}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <Link href={`/admin/content/herbs/${item.id}`} className={t.btnOutlineGreen}>Edit</Link>
                      <button
                        type="button"
              ...
              {items.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-4 py-8 text-center text-sm text-gray-400">
                    {search || statusFilter ? "No herb profiles match your filters." : "No herb profiles yet. Create your first one."}
                  </td>
                </tr>
              )}
            </tbody>
